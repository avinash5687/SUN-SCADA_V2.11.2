...
const SMB_Heatmap = () => {
  ...
  const fetchSmbData = useCallback(async (isRefresh = false) => {
    try {
      ...
      if (Array.isArray(response.data) && response.data.length > 0) {
        ...
        console.warn("⚠️ No SMB data received");
      }
  ...
  // ✅ UPDATED: Remove rows after Power (Voltage, INT Temp, EXT Temp, Panel Temp removed)
  const parameters = [
  ...
  return (
    <div className="smb-container">
      {/* Page Header Section - Page name left, selection right */}
      <div className="smb-header-section">
        <div className="smb-header-left">
          <h2 className="smb-title">SMB Heatmap</h2>
        </div>
      ...
      {/* Legends Section - Full width row */}
      <div className="smb-legend-section">
        <div className="smb-legend">
          <span><b>Deviation Legend:</b></span>
          <span className="legend-item"><span className="dot green"></span> Excellent (0–25%)</span>
          <span className="legend-item"><span className="dot yellow"></span> Good (25–50%)</span>
          <span className="legend-item"><span className="dot orange"></span> Warning (50–75%)</span>
          <span className="legend-item"><span className="dot red"></span> Critical (75%+)</span>
        </div>
      </div>
                ...
                {parameters.map((param, idx) => (
                  <tr key={idx}>
                    ...
                    {smbIds.map(id => {
                      ...
                      if (!smb) return <td key={id} className="no-data-cell">No Data</td>;
                      ...
                      if (param.key.startsWith("string_current")) {
                        ...
                        return (
                          <td key={id} className={`deviation-cell ${className}`}>
                            {value ?? "No Data"}
                          </td>
                        );
                      }
